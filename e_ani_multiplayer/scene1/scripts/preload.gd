...
func preload_cam():
	if(is_network_master()):
		global_v.main_cam=global_v.get_node("main_camera")
		global_v.second_cam=global_v.get_node("spayer_camera")
	else:
		global_v.main_cam=global_v.get_node("spayer_camera")
		global_v.second_cam=global_v.get_node("main_camera")
	global_v.main_cam.current=false
	global_v.get_node("preload/preload_camera").current=true
	global_v.get_node("preload/preload_camera").make_current()

func unload_cam():
	global_v.main_cam.current=true
	global_v.get_node("preload/preload_camera").current=false
	global_v.main_cam.make_current()
...
func preload_parts():
	if(!is_instance_valid(ael)):
		...
		if cpart<elems.size():
			...
			fael=false
			rec_s(ael)
			self.call_deferred("add_child",ael,true)
			cpart+=1
...
var server_loading=true
var client_loading=true
onready var status_t=global_v.get_node("preload_h/progress/VBoxContainer/Label4")
master func client_end():
	client_loading=false
puppet func server_end():
...
func _process(delta):
	if(!once)||(preload_done):
		...
		if(client_loading):
			if is_network_master():
				status_t.text="Waiting for client loading..."
				return
		if(server_loading):
			if !is_network_master():
				status_t.text="Waiting for server loading..."
				return
		finishx()
		return
	preload_parts()
	...
	if(global_v.iTime>25):
		status_t.text="Error: Preload long or failed, some effects may be broken. Returning to game."
