...
func _ready():
	...
	tv=smoothstep(5.0,0.0,tv)*shoot_mov*1.25
...
func _on_b1_body_entered(body):
	...
	if(body.is_in_group("chess")||body.is_in_group("Enemy")||body.is_in_group("wall")||body.is_in_group("dyn_obj")):
		...
		is_hit=true
		return
	if(!body.is_in_group("ignore"))&&(!body.is_in_group("player"))&&(!body.is_in_group("player2")):
		imp(body,(-get_node("../").translation+body.translation))
		del_it=true
		is_hit=true
		return
...
puppet func upd_futari_exp(is_hitx):
	if(is_hitx)&&(!no_parts):
		get_node("../futari/on_hit").emitting=true
	get_node("../model").get_child(0).emitting=false
	get_node("../model").get_child(1).emitting=false
...
func _process(delta):
	...
	if(del_it):
		if(once):
			...
			get_node("../model").get_child(1).emitting=false
			get_node("../futari/f").emitting=false
			rpc("upd_futari_exp",is_hit)
			once=false
		del_timer+=delta
		if(del_timer>5):
			rpc("free_shoot")
	else:
		var t=max(8.0*(1.0/shoot_mov),0.06)
		...
		get_node("../").translate(forward_dir * (extra_speed+(min_mov+(shoot_mov-min_mov)*smoothstep(0.05,01.35,start_time))) * delta)
