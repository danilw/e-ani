...
puppet func add_chess_client_multiplayer(transl,pause_time,self_idx,
	self_id,bonus_in_i,self_mat,rand_rot,ches_idsl,ernd):
	var s=elems[self_id].instance()
	s.translation.z=transl.z
	s.translation.x=transl.x
	s.pause_time=pause_time
	s.self_idx=self_idx
	s.self_id=self_id
	var sbon=null
	if(bonus_in_i>=0):
		sbon=elems_bonus[bonus_in_i].instance()
		sbon.self_type=bonus_in_i
	s.bonus_in=sbon
	s.b_mat=b_mat
	s.self_mat=self_mat
	s.rand_rot=rand_rot
...
func upd_child_unlag(frame):
	if(int(frame)%65==0):
		...
		for a in range(min(3,osz)):
			var tss=Node.new()
			tss.name="cont"+str(global_v.ches_ids)
			tss.add_child(que[osz-a-1],true)
			self.call_deferred("add_child",tss,true)
			var s=que[osz-a-1]
			var bon=-1
			if(s.bonus_in!=null):
				bon=s.bonus_in.self_type
			rpc("add_chess_client_multiplayer",s.translation,s.pause_time,s.self_idx,s.self_id,bon,s.self_mat,s.rand_rot,global_v.ches_ids,s.ernd)
			global_v.ches_ids+=1
	...
	for a in range(sz.x):
		for b in range(sz.y):
			if(board_map[a][b][2])&&(!board_map_tmp[a][b][2]):
				...
				s.b_mat=b_mat
				s.ernd=int(rand_range(0,s.self_id))+int(global_v.ground/3)
				if(b-4>=0):
					s.self_mat=0
				else:
					s.self_mat=1
