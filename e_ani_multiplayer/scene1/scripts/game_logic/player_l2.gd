...
var inp_state_l=false
var inp_state_r=false
var inp_state_u=false
var inp_state_d=false
var inp_state_j=false
...
func _ready():
	...
	global_v.player_pos2=self.translation
...
master func upd_inp_state(a,b,c,d,e):
	inp_state_l=c
	inp_state_r=d
	inp_state_u=b
	inp_state_d=a
...
puppet func update_tranform(transformx,sr,sttr,sl,sttl):
	self.transform=transformx
	global_v.player_pos2=self.translation
	global_v.player_rot2=self.rotation_degrees.y
	shoot_timer_r=sr
	shoot_timeout_r=sttr
	shoot_timer_l=sl
	shoot_timeout_l=sttl
...
puppet func gen_shoot_l_m(angle,start_pos,aim_point_pos,extra_speed,aie_e,min_mov,shoot_mov,self_dmg,tidsx_ll):
	...
	global_v.get_node("shoot_l2").call_deferred("add_child",tss,true)
...
puppet func gen_shoot_r_m(angle,start_pos,aim_point_pos,extra_speed,aie_e,min_mov,shoot_mov,self_dmg,tidsx_rl):
	...
	tss.add_child(s,true)
	global_v.get_node("shoot_r2").call_deferred("add_child",tss,true)

func _process(delta):
	if !(is_network_master()):
		rpc("upd_mouse_state",Input.is_mouse_button_pressed(BUTTON_LEFT),Input.is_mouse_button_pressed(BUTTON_RIGHT))
		rpc("upd_inp_state",Input.is_action_pressed("ui_down"),
	...
	rpc("update_tranform",self.transform,shoot_timer_r,shoot_timeout_r,shoot_timer_l,shoot_timeout_l)
...
func process_shoot_l(delta):
	shoot_timer_l+=delta
	if(shoot_timer_l>=shoot_timeout_l)&&(iMoused.x>0.99)&&(mosue_state_l):
		shoot_timer_l=0.0
		obc=true
	else:
...
func process_shoot_r(delta):
	shoot_timer_r+=delta
	if(shoot_timer_r>=shoot_timeout_r)&&(iMoused.y>0.99)&&(mosue_state_r):
		shoot_timer_r=0.0
		obc2=true
	else:
...
func gen_shoot_l():
	...
	tmp_shoot_l[idx].get_node("b1").aim_point_pos=global_v.iMouse_3d2
	tmp_shoot_l[idx].get_node("b1").extra_speed=max(-local_speed,0)
	tmp_shoot_l[idx].get_node("b1").aie_e=global_v.mouse_aim_e2
...
func gen_shoot_r():
	...
	tmp_shoot_r[idx].get_node("b1").aim_point_pos=global_v.iMouse_3d2
	tmp_shoot_r[idx].get_node("b1").extra_speed=max(-local_speed,0)
	tmp_shoot_r[idx].get_node("b1").aie_e=global_v.mouse_aim_e2
...
func process_input(delta):
	...
	if mosue_state_l&&(!global_v.game_over):
		if(!iMousel):
		...
		iMousel=false
	if mosue_state_r&&(!global_v.game_over):
		if(!iMouser):
