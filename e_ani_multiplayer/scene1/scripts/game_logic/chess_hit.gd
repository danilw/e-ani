...
onready var ernd=get_node("../../").ernd
...
remotesync func spwn_bonus(bonus_idsl):
	if(bonus_in==null):
		return
...
func check_alive():
	...
	if (is_network_master()):rpc("hit_bs")


remotesync func hit(dmg,pos,enemys_idsl,rnd,rnd2):
	if (!is_network_master()):
		print("debug cliend ID: "+str(enemys_idsl))
	if(!is_alive):
		return
	#hpx+=-dmg
	hpx+=-1
	...
	if(!is_alive):
		en_spwn.load_new(get_node("../../").translation,enemys_idsl,rnd,rnd2)
		global_v.board_map[get_node("../../").self_idx.x][get_node("../../").self_idx.y][2]=false
		if (is_network_master()):check_alive()
		self.visible=false
		get_node("../hp").visible=false
		get_node("CollisionShape").disabled=true
		if (is_network_master()):
			rpc("spwn_bonus",global_v.bonus_ids)
...
func test_hit(pos, dmg):
	if(is_anim_end):
		if (is_network_master()):
			rpc("hit",dmg,pos,global_v.enemys_ids,int(rand_range(0,2.9)),randf()>0.5)
			print("debug host id: "+str(global_v.enemys_ids))
			global_v.enemys_ids+=1
...
func _process(delta):
	...
	if(!is_alive):
		del_timer+=delta
	if(del_timer>25.0):
		if (is_network_master()):rpc("free_block")
