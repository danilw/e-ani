...
onready var anim_tree=get_tree().get_root().get_node("scene/main_player/model/AnimationTree")
...
func _process(delta):
#	shoot_timeout_l=tl*0.2+tl*0.8*smoothstep(20.0,10.0,global_v.iTime) #debug
#	shoot_timeout_r=tr*0.5+tr*0.5*smoothstep(20.0,10.0,global_v.iTime)
...
func gen_shoot_l():
	...
	global_v.call_deferred("add_child",tmp_shoot_l[idx])
...
func gen_shoot_r():
	...
	global_v.call_deferred("add_child",tmp_shoot_r[idx])
